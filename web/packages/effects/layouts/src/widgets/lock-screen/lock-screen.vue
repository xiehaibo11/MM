...
async function checkPassword(password: string) {
  if (props.validatePassword) {
    return await props.validatePassword(password);
  }
  return !!lockScreenPassword?.value && lockScreenPassword.value === password;
}
    ...
    <transition name="slide-right">
      ...
      >
        <div class="mb-10 flex-col-center w-[90%] max-w-75 px-4">
          <VbenAvatar :src="avatar" class="enter-x mb-4 size-20" />
          <div
            v-if="username"
            class="enter-x mb-5 max-w-full truncate text-center text-base font-medium text-foreground"
          >
            {{ username }}
          </div>
