...
const form = reactive({ usrname: '', email: '' })
const busy = ref(false)
const mode = ref<'usrname' | 'email'>('usrname')

async function submit() {
  const target = mode.value === 'usrname'
  ...
  try {
    ...
    if (res.error || res.status === 'fail') throw new Error(res.error || res.message || '操作失败')
    message.success(`已转让到 ${res.email || target.usrname || target.email}`)
    emit('done')
  } catch (err) {
    message.error(apiError(err, '指派失败'))
  } finally {
    busy.value = false
  }
}
  ...
  >
    ...
    <a-form layout="vertical">
      ...
      <a-form-item v-else label="目标邮箱">
        <a-input v-model:value="form.email" placeholder="user@example.com" autofocus />
      </a-form-item>
      <p class="warn">
        提示：当前归属 <code>{{ device.usrname }}</code>，转让后原用户立即失去访问权限。
      </p>
    </a-form>
  </a-modal>
</template>
...
<style scoped>
.warn { font-size: 12px; color: var(--text-2); margin: 0; }
.warn code { color: var(--accent-pink); }
</style>
